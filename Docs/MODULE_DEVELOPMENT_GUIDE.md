...
- entregarse siempre dentro del ZIP completo de ZEUVE.
...
Después actualiza `Scripts/generate_xcode_project.py` para que el producto se enlace al target de la aplicación y regenera `ZEUVE.xcodeproj`.
...
En el arranque de la aplicación, registra el manifiesto en la instancia compartida de `ModuleRegistry`. El registro debe fallar con un mensaje comprensible si:
...
No registres un módulo que todavía no tenga funcionalidad real ni navegación disponible.
...
9. finalizar el coordinador;
10. guardar historial y mostrar resumen.
...
Usa `HistoryRepository` o un servicio del módulo que lo encapsule. El historial no debe guardar contenido personal completo si basta con metadatos operativos.
...
La entrega es siempre el proyecto completo en ZIP, sin `.build`, `build`, `.DS_Store`, `__MACOSX`, `xcuserdata`, logs ni datos privados.

## 18. Lista de integración rápida

1. Completar el brief del módulo.
2. Analizar el ZIP más reciente.
...
8. Registrar el manifiesto.
9. Añadir navegación únicamente cuando funcione.
10. Integrar operación, progreso, cancelación, logs e historial.
11. Regenerar Xcode.
12. Ejecutar pruebas y compilaciones.
13. Actualizar versión y documentación.
14. Empaquetar el proyecto completo y limpio.
