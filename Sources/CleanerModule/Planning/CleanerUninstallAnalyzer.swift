...
public struct CleanerUninstallAnalyzer{
    let fileManager:FileManager;let identityProvider:any CleanerAppIdentityProviding;let associationService:CleanerAssociationService
    public init(fileManager:FileManager = .default,identityProvider:any CleanerAppIdentityProviding=SystemCleanerAppIdentityProvider(),associationService:CleanerAssociationService?=nil){self.fileManager=fileManager;self.identityProvider=identityProvider;self.associationService=associationService ?? CleanerAssociationService(fileManager:fileManager)}
    public func analyze(application:CleanerAppInventoryItem,historicalApps:[CleanerAppInventoryItem],keptPaths:Set<String>,safeSelection:Bool)->CleanerUninstallAnalysis{
        ...
        let association=associationService.scan(installedApps:[application],historicalApps:Array(Set(historicalApps+[application])),keptPaths:keptPaths,runningBundleIDs:running ? Set([bundle].compactMap{$0}):[])
        ...
        var related=association.candidates.filter{ candidate in
            bundle.map { candidate.associatedBundleID == $0 }
                ?? (candidate.associatedBundleID == nil && candidate.associatedAppName == application.identity.name)
        }.map{ c in CleanerPlanner.copy(c,selected:safeSelection && c.canBeSafelyPreselected) }
