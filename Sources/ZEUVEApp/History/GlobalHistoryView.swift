...
private enum GlobalHistoryLoadOutcome: Sendable {
    case success([GlobalHistoryEntry])
    case failure(String)
}
...
final class GlobalHistoryViewModel: ObservableObject {
    ...
    init(storage: StorageContainer?) {
        ...
        presenters = BuiltInModuleCatalog.historyPresenters
    }
    ...
    func load(manifests: [ModuleManifest]) {
        ...
        let historyManifests = BuiltInModuleCatalog.historyManifests(from: manifests)
        loadTask = Task { [weak self] in
            let result = await Self.loadEntries(
                service: service,
    ...
    nonisolated private static func loadEntries(
        ...
        moduleID: String?
    ) async -> GlobalHistoryLoadOutcome {
        await Task.detached(priority: .userInitiated) {
            do {
                let identifiers = BuiltInModuleCatalog.historyIdentifiers(forSelectedIdentifier: moduleID)
                if identifiers.count == 1 {
                    return .success(try service.entries(
                        manifests: manifests,
                        presenters: presenters,
                        moduleID: identifiers[0],
                        limit: 500
                    ))
                }

                var combined: [GlobalHistoryEntry] = []
                for identifier in identifiers.compactMap({ $0 }) {
                    combined.append(contentsOf: try service.entries(
                        manifests: manifests,
                        presenters: presenters,
                        moduleID: identifier,
                        limit: 500
                ...
                let entries = combined.sorted { $0.record.createdAt > $1.record.createdAt }.prefix(500).map { $0 }
...
struct OrganizerGlobalHistoryPresenter: ModuleHistoryPresenter {
