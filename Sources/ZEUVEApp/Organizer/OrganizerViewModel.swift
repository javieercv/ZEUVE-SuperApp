...
final class OrganizerViewModel: ObservableObject {
    ...
    init(
        ...
        let savedDefaults = (try? settings.value(forKey: "organizer.defaultOptions", as: OrganizerOptions.self))
            ?? (try? settings.value(forKey: "organizer.options", as: OrganizerOptions.self))
        if let savedDefaults {
            defaultOptions = savedDefaults
            options = savedDefaults
        }
        if let recent = try? settings.value(forKey: "organizer.lastFolder", as: String.self) {
            folderURL = URL(fileURLWithPath: recent, isDirectory: true)
        }
        let recentPaths = (try? settings.value(forKey: "organizer.recentFolders", as: [String].self)) ?? []
    ...
    func persistDefaultOptions() {
        try? settings?.set(defaultOptions, forKey: "organizer.defaultOptions")
    }
    ...
    func clearRecentFolders() {
        recentFolders = []
        try? settings?.set([String](), forKey: "organizer.recentFolders")
        try? settings?.removeValue(forKey: "organizer.lastFolder")
    }
    ...
    func setFolder(_ url: URL) {
        ...
        state = .idle
        try? settings?.set(url.standardizedFileURL.path, forKey: "organizer.lastFolder")
        ...
        try? settings?.set(recentFolders.map(\.path), forKey: "organizer.recentFolders")
    }
        ...
        operationTask = Task { [weak self] in
            ...
            do {
                ...
                _ = try? await logger?.write(
                    ...
                    metadata: ["carpeta": folderURL.path]
                )
                ...
                _ = try? await logger?.write(
                    ...
                    metadata: ["carpeta": folderURL.path, "operaciones": "\(newPlan.operations.count)", "omitidos": "\(newPlan.ignored.count)"]
                )
    ...
    func execute() {
        ...
        operationTask = Task { [weak self] in
            ...
            do {
                ...
                currentOperationID = nil
                executionWorker = nil
                let savedRecord = try await Task.detached { try history.save(result) }.value
                self.plan = nil
                selectedIDs.removeAll()
                progress = nil
                state = .idle
                ...
                _ = try? await logger?.write(
                    ...
                    metadata: ["carpeta": result.baseFolder.path, "movidos": "\(result.moved)", "omitidos": "\(result.skipped)", "renombrados": "\(result.renamed)"]
                )
