...
final class UniversalConverterViewModel: ObservableObject {
    ...
    private var outputFolderAccess: SecurityScopedResourceAccess?
        ...
        executionTask = Task { [weak self] in
            ...
            do {
                ...
                let result = try await execution.execute(
                    plan,
                    archivePassword: password.isEmpty ? nil : password,
    ...
    func restorePersistentDefaultsForGlobalReset() -> [String] {
        ...
        restoreDefaults()
        if let errorMessage {
            failures.append("Conversor universal — valores predeterminados: \(errorMessage)")
            self.errorMessage = nil
        }
        do {
            try bookmarkStore?.clear()
        } catch {
            failures.append("Conversor universal — carpeta de salida recordada: \(error.localizedDescription)")
        }
        return failures
    }
...
extension UniversalConverterViewModel {
    ...
    private func setOutputFolder(_ url: URL, persistBookmark: Bool) {
        do {
            ...
            outputFolderAccess = SecurityScopedResourceAccess(valid)
            outputFolder = valid
