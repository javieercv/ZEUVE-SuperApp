...
public actor ExternalProcessRunner {
    ...
    public func run(
        ...
        let waitResult = await withTaskCancellationHandler {
            await Task.detached(priority: .userInitiated) { () -> (Int32, Int32, Int32) in
                var exitStatus: Int32 = -1
                var signal: Int32 = 0
                let code = Int32(zeuve_wait_process(waitedPID, -1, &exitStatus, &signal))
    ...
    private static func readTask(fd: Int32, handler: @escaping OutputHandler) -> Task<Void, Never> {
        Task.detached(priority: .utility) {
            ...
            while true {
                ...
                if count > 0 {
                    #if canImport(Darwin)
                    // Las tareas detached no drenan un run loop: liberar NSData por bloque
                    // impide retener todo el PCM de una película como temporales autoreleased.
                    autoreleasepool { handler(Data(buffer[0..<count])) }
                    #else
