...
public actor UniversalConverterExecutionService {
    ...
    public init(
        ...
        self.runner = runner
        self.probeService = MediaProbeService()
    ...
    public func execute(
        ...
        do {
            ...
            requirements.calibre = plan.items.contains { $0.executionKind == .calibre }
            requirements.ghostscript = plan.items.contains { $0.executionKind == .ghostscript }
            let needsExternalEngine = requirements.media || requirements.pandoc || requirements.calibre || requirements.ghostscript
            ...
            _ = try? history?.save(result, plan: plan)
    ...
    private func executeItem(
        ...
        case .ffmpeg:
            ...
            if item.operation == .imagesToVideo {
                ...
                source = firstSource
                probe = try await probeService.probe(url: source, ffprobe: ffprobe, fingerprint: item.sources.first?.kind == .file ? item.sources.first?.fingerprint : nil)
            }
        ...
        case .calibre:
            guard let executable = engines?.calibre else { throw UniversalConverterError.dependencyMissing("Calibre") }
            guard let source = sources.first else { throw UniversalConverterError.noInput }
            let output = try workspace.generatedURL(relativePath: item.destinationRelativePath)
            try await runExternal(
                CalibreCommandBuilder().request(executable: executable, source: source, target: item.targetFormat, destination: output, options: options),
                engineName: "Calibre",
                progress: progress
            )
            try await resultValidator.validate(url: output, expectedFormat: item.targetFormat, directory: false, ffprobe: engines?.ffprobe)
        ...
        case .ghostscript:
            guard let executable = engines?.ghostscript else { throw UniversalConverterError.dependencyMissing("Ghostscript") }
            guard let source = sources.first else { throw UniversalConverterError.noInput }
            let output = try workspace.generatedURL(relativePath: item.destinationRelativePath)
            try await runExternal(
                GhostscriptCommandBuilder().request(executable: executable, source: source, target: item.targetFormat, destination: output, options: options),
...
private final class FFmpegDiagnosticCollector: @unchecked Sendable {
...
private struct ItemProgressUpdate: Sendable {
    let fraction: Double?
    let phase: String
}

private final class FFmpegProgressMonitor: @unchecked Sendable {
    private let lock = NSLock()
    private let decoder = IncrementalLineDecoder(maximumBufferedBytes: 512 * 1_024)
