...
public struct ConverterEngineAvailability: Codable, Sendable, Equatable {
    ...
    public var calibre: Bool
    public var ghostscript: Bool
    ...
    public static let logicalTestEnvironment = ConverterEngineAvailability(
        ...
        pandoc: true, calibre: true, ghostscript: true, videoToolbox: true,
...
public struct ConverterCompatibilityRegistry: Sendable {
    ...
    public func supports(from source: ConverterFormat, to target: ConverterFormat, operation: ConversionOperation = .convert) -> Bool {
        if source == target, operation == .convert { return true }
    ...
    private static func makeEntries() -> [ConverterCompatibilityEntry] {
        ...
        addMatrix(markup, [.txt, .markdown, .html, .epub], engine: .pandoc, risks: [.layout, .metadata])

        let ebooks: [ConverterFormat] = [.epub, .mobi, .azw3, .fb2]
        addMatrix(ebooks, [.epub, .mobi, .azw3, .fb2, .html, .txt, .pdf], engine: .calibre, risks: [.layout, .metadata, .unsupportedFeatures])

        for target in raster {
            values.append(.init(inputFormat: .eps, outputFormat: target, engine: .ghostscript, supportedOptions: ["DPI", "fondo", "antialiasing"], lossRisks: [.vectorRasterization, .metadata]))
        ...
        values.append(.init(inputFormat: .eps, outputFormat: .pdf, engine: .ghostscript, supportedOptions: ["conservar vectorial"], lossRisks: [.unsupportedFeatures]))
