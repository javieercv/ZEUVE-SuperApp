...
public struct WhatsAppImporter: Sendable {
    ...
    public func inspectArchive(at url: URL, cancellation: @Sendable () -> Bool = { false }) throws -> WhatsAppInspection {
        let reader = ChatArchiveReader(url: url, limits: settings.archiveLimits)
        let catalog = try reader.catalog(cancellation: cancellation)
        let candidates = catalog.entries
            .filter { !$0.isDirectory && $0.path.lowercased().hasSuffix(".txt") }
            .map(\.path)
        let samples = try reader.readPrefixes(paths: Set(candidates), maximumBytesPerEntry: 256 * 1_024, cancellation: cancellation)
    ...
    public func importArchive(
        ...
        let inspection = try inspectArchive(at: url, cancellation: cancellation)
        let selected: String
        ...
        return try parse(
            ...
            try reader.stream(path: selected, maximumBytesPerEntry: settings.conversationFileMaximumBytes, cancellation: cancellation) { chunk in
    ...
    public func streamArchive(
        ...
        batchSize: Int = 4_096,
        cancellation: @Sendable () -> Bool = { false },
        consume: @escaping ([NormalizedMessage]) throws -> Void
        ...
        let reader = ChatArchiveReader(url: url, limits: settings.archiveLimits)
        var buffer: [NormalizedMessage] = []
        buffer.reserveCapacity(max(1, batchSize))
        let result = try parse(
            ...
            emit: { message in
                ...
                if buffer.count >= max(1, batchSize) {
                    try consume(buffer)
            ...
            try reader.stream(path: selected, maximumBytesPerEntry: settings.conversationFileMaximumBytes, cancellation: cancellation) { chunk in
                if cancellation() { throw ChatAnalyzerError.cancelled }
                try decoder.append(chunk, consume: lineConsumer)
            }
            try decoder.finish(consume: lineConsumer)
        }
    ...
    private func parse(
        ...
        collectMessages: Bool = true,
        emit: ((NormalizedMessage) throws -> Void)? = nil,
        produceLines: (_ consume: (String) throws -> Void) throws -> Void
    ) throws -> WhatsAppImportResult {
        var messages: [NormalizedMessage] = []
        var recognizedMessages = 0
        var missingAttachments = 0
        var hasUncertainDate = false
        ...
        func flush() throws {
            ...
            recognizedMessages += 1
            if attachmentsVerified, attachment != nil, attachment?.exists == false { missingAttachments += 1 }
            hasUncertainDate = hasUncertainDate || item.uncertainDate
            if collectMessages { messages.append(message) }
            try emit?(message)
        }
        ...
        try flush()
        guard recognizedMessages > 0 else { throw ChatAnalyzerError.whatsappNoMessages }
        if collectMessages { messages.sort { ($0.timestamp, $0.sourcePosition) < ($1.timestamp, $1.sourcePosition) } }

        let missing = attachmentsVerified ? missingAttachments : 0
        ...
        summary.recognizedMessages = recognizedMessages
