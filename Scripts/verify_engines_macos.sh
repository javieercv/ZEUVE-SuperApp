...
expected_names = {'yt-dlp','deno','ffmpeg','ffprobe','pandoc','calibre','ghostscript'}
names = set()
zeros = '0' * 64
bundle_roots = {
    'yt-dlp': pathlib.PurePosixPath('yt-dlp'),
    'pandoc': pathlib.PurePosixPath('pandoc'),
    'calibre': pathlib.PurePosixPath('calibre/Calibre.app'),
    'ghostscript': pathlib.PurePosixPath('ghostscript'),
}
for item in items:
...
assert names == expected_names, f"Registro inesperado de motores: {sorted(names)}"
PY
...
PY
  ...
  case "$name" in
    yt-dlp)
      ...
      ;;
    deno|pandoc|calibre|ghostscript) output="$("$path" --version 2>&1)" ;;
  ...
  case "$name" in
    calibre)
      /usr/bin/codesign --verify --deep --strict --verbose=2 "$ENGINES/calibre/Calibre.app"
      continue
      ;;
  esac
...
if [[ "$VERIFY_PROJECT_LOCATION" == "1" ]]; then
  ...
  find "$ROOT" -path '*/.build' -prune -o -path '*/.engine-build' -prune -o -type f \( -name ffmpeg -o -name ffprobe \) -print > "$paths_file"
