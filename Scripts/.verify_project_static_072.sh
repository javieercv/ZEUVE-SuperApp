...
from pathlib import Path
import json
assert Path('VERSION').read_text().strip() == '0.7.2'
pbx = Path('ZEUVE.xcodeproj/project.pbxproj').read_text()
for required in ['MARKETING_VERSION = 0.7.2;', 'CURRENT_PROJECT_VERSION = 23;', 'ENABLE_HARDENED_RUNTIME = YES;', 'ZEUVEEngines', 'YouTubeDownloaderModule', 'ChatAnalyzerModule', 'UniversalConverterModule', 'Engines in Resources']:
...
assert converter['version'] == '0.2.0'
...
assert engine_names == {'yt-dlp','deno','ffmpeg','ffprobe','libreoffice','pandoc','calibre','ghostscript'}
yt_dlp = next(item for item in engines['engines'] if item['name'] == 'yt-dlp')
for name in {'libreoffice','pandoc','calibre','ghostscript'}:
...
if 'libreoffice/LibreOffice.app/Contents/MacOS/soffice' not in preserved:
    raise SystemExit('LibreOffice no figura entre los paquetes cuya firma oficial debe conservarse.')
...
for required in ['codesign --verify --strict', '"$YTDLP" --version', 'YTDLP_STATUS', 'LibreOffice.app', '--headless --version']:
...
for value in ['h264_videotoolbox', 'fps_mode', 'passthrough', 'tiempos.csv', 'ImageIO', 'PDFKit', '--headless', 'ConverterSafePath', 'revision']:
