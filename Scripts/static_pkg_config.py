...
para las bibliotecas estáticas que se compilan junto a la app.
"""
...
def load_package(name: str) -> Package:
    ...
    for directory in search_paths:
        ...
        return Package(
            ...
            requires=expand(fields.get("Requires", ""), variables),
            requires_private=expand(fields.get("Requires.private", ""), variables),
            variables=variables,
        )
...
def resolve_packages(requirements: list[Requirement], static: bool) -> list[Package]:
    packages: list[Package] = []
    seen: set[str] = set()

    def visit(requirement: Requirement) -> None:
        if requirement.package in seen:
            return

        package = load_package(requirement.package)
        seen.add(package.name)
        packages.append(package)

        dependency_tokens = split_package_list(package.requires)
        if static:
            dependency_tokens.extend(split_package_list(package.requires_private))
        dependency_requirements = parse_requirements(
            normalize_requirement_arguments(dependency_tokens)
        )
...
def main(argv: list[str]) -> int:
    ...
    try:
        packages = resolve_packages(requirements, static)
