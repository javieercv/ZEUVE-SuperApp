...
def _chat_results_source() -> str:
    root = ROOT / "Sources/ZEUVEApp/ChatAnalyzer"
    files = [root / "ChatAnalyzerResultsView.swift"] + sorted((root / "Results").glob("*.swift"))
    return "\n".join(path.read_text() for path in files)


def _chat_analytics_source() -> str:
    root = ROOT / "Sources/ChatAnalyzerModule/Analysis"
    return "\n".join(path.read_text() for path in sorted(root.glob("*.swift")))
...
class ChatAnalyzerUIRulesTests(unittest.TestCase):
    ...
    def test_chat_results_use_background_snapshots_instead_of_recalculating_in_views(self) -> None:
        ...
        self.assertIn("detachedStoreAnalyticsValue", model)
        self.assertIn("Task.sleep(nanoseconds: 200_000_000)", model)
        self.assertIn("worker.cancel()", model)
        self.assertIn("ChatStoreCoreAnalyticsSnapshot", analytics)
        ...
        self.assertNotIn("coreSnapshot.filteredMessages", model)
        self.assertNotIn("coreSnapshot.messages", model)
        self.assertNotIn("searchTextCache", model)

    def test_background_refresh_cancels_stale_work_and_avoids_unrelated_core_rebuilds(self) -> None:
        model = (ROOT / "Sources/ZEUVEApp/ChatAnalyzer/ChatAnalyzerViewModel.swift").read_text()
        analytics = _chat_analytics_source()
