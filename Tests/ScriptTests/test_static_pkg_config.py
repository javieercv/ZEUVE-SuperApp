...
class StaticPkgConfigTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        (self.pkgconfig / "libsharpyuv.pc").write_text(
            "\n".join(
                [
                    f"prefix={prefix}",
                    "exec_prefix=${prefix}",
                    ...
                    "",
                    "Name: libsharpyuv",
                    "Description: Sharp YUV conversion",
                    "Version: 1.6.0",
        ...
        (self.pkgconfig / "libwebp.pc").write_text(
            "\n".join(
                [
                    f"prefix={prefix}",
                    "exec_prefix=${prefix}",
    ...
    def test_static_libs_include_requires_private_packages(self) -> None:
        result = self.run_tool("--static", "--libs", "libwebp")
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("-lwebp", result.stdout)
        self.assertIn("-lsharpyuv", result.stdout)
