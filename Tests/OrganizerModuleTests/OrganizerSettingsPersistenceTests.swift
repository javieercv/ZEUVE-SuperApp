...
final class OrganizerSettingsPersistenceTests: XCTestCase {
    func testDefaultOptionsRemainIndependentFromLegacyOperationOptions() throws {
        ...
        try storage.settings.set(defaults, forKey: "organizer.defaultOptions")
        try storage.settings.set(operation, forKey: "organizer.options")

        XCTAssertEqual(
            try storage.settings.value(forKey: "organizer.defaultOptions", as: OrganizerOptions.self),
            defaults
        )
        XCTAssertEqual(
            try storage.settings.value(forKey: "organizer.options", as: OrganizerOptions.self),
            operation
        )
    }
}
