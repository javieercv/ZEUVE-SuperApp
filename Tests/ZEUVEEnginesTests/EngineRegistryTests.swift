...
final class EngineRegistryTests: XCTestCase {
    ...
    func testUniversalYTDLPArchitectureHeadersDoNotBlockDiagnostics() async throws {
        #if os(macOS) && arch(arm64)
        let project = URL(fileURLWithPath: #filePath).deletingLastPathComponent().deletingLastPathComponent().deletingLastPathComponent()
        let engineRoot = project.appendingPathComponent("Resources/Engines")
        let executable = engineRoot.appendingPathComponent("yt-dlp/yt-dlp_macos")
        guard FileManager.default.fileExists(atPath: executable.path) else {
            throw XCTSkip("Los motores locales no están presentes en esta copia.")
        }
        guard MachOInspector.satisfies(.universal, at: executable) else {
            throw XCTSkip("yt-dlp no es universal en esta copia.")
        }
        let registry = try EngineRegistry(resourceRoot: engineRoot)
        let optionalDiagnostic = await EngineDiagnosticService(registry: registry).diagnose(named: "yt-dlp")
        let diagnostic = try XCTUnwrap(optionalDiagnostic)

        XCTAssertEqual(diagnostic.state, .ready)
        XCTAssertFalse(diagnostic.dynamicDependencies.contains { $0.hasPrefix("FALTA:") })
        #else
        throw XCTSkip("El diagnóstico ejecutable de yt-dlp requiere macOS Apple Silicon.")
        #endif
    }
    ...
    func testEngineOverridePreservesSourceAndCanRestoreBundledSelection() throws {
        let overrides = root.appendingPathComponent("overrides", isDirectory: true)
        let source = root.appendingPathComponent("tool")
        try Data("#!/bin/sh\necho 2.0\n".utf8).write(to: source)
        try FileManager.default.setAttributes([.posixPermissions: 0o755], ofItemAtPath: source.path)
        let originalHash = try SHA256.hexDigest(fileAt: source)
        let manager = EngineOverrideManager(root: overrides)
        let record = try manager.installLocalExecutable(
            named: "tool",
            version: "2.0",
            source: source,
            channel: .stable,
        ...
        XCTAssertEqual(record.sha256, originalHash)
        XCTAssertEqual(try manager.activeExecutable(named: "tool").map { try SHA256.hexDigest(fileAt: $0) }, originalHash)
        XCTAssertEqual(try SHA256.hexDigest(fileAt: source), originalHash)
        try manager.restoreBundled(named: "tool")
        XCTAssertNil(try manager.activeExecutable(named: "tool"))
        XCTAssertTrue(EngineOverrideManager.updateWarning.localizedCaseInsensitiveContains("estable") == false)
        XCTAssertTrue(EngineOverrideManager.updateWarning.contains("Actualizar"))
    }
